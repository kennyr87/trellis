...
def get_version
  s = Vagrant::Util::Subprocess.execute("virsh", '--version')
  s.stdout.strip!
end
...
Vagrant.configure("2") do |config|
  ...
  machine_name = vconfig.fetch('vagrant_machine')
  ...
  config.vm.define machine_name do |node|
    node.vm.hostname = main_hostname
    node.vm.network :private_network, ip: vconfig.fetch('vagrant_ip'), hostsupdater: 'skip'
    node.hostmanager.aliases = hostnames + redirects
  ...
  config.vm.provision provisioner do |ansible|
    ...
    config.vm.provider :libvirt do |libvirt|
      libvirt.memory = vconfig.fetch('vagrant_memory')
      libvirt.cpus = vconfig.fetch('vagrant_cpus')
    end
  end
end